...
module.exports = (function() {
  ...
  **/
  ...
  QueryInterface.prototype.addIndex = function(tableName, _attributes, options, _rawTablename) {
    ...
    return this.sequelize.query(sql, { logging: options.logging, dbName: options.dbName });
  };
  ...
  QueryInterface.prototype.getForeignKeysForTables = function(tableNames, options) {
    ...
    return Promise.map(tableNames, function(tableName) {
      return self.sequelize.query(self.QueryGenerator.getForeignKeysQuery(tableName, self.sequelize.config.database), {logging: options.logging, dbName: options.dbName}).get(0);
  ...
  QueryInterface.prototype.rawSelect = function(tableName, options, attributeSelector, Model) {
    ...
    var sql = this.QueryGenerator.selectQuery(tableName, options, Model)
      , queryOptions = Utils._.extend({ transaction: options.transaction, plain: options.plain, logging: options.logging, dbName: options.dbName }, { raw: true, type: QueryTypes.SELECT });

    if (attributeSelector === undefined) {
      throw new Error('Please pass an attribute selector!');
    }

    return this.sequelize.query(sql, queryOptions).then(function(data) {
  ...
  QueryInterface.prototype.setAutocommit = function(transaction, value, options) {
    ...
    if (sql) {
      return this.sequelize.query(sql, { transaction: transaction, logging: options.logging, dbName: options.dbName });
    } else {
      return Promise.resolve();
    }
  };
