...
var ConnectionPool = function(config, dialect) {
  ...
  this.pools = {};
	...
	// Duplicate the base config into each shard
	var defaultConfig = _.extend({}, config);
	delete defaultConfig.shards; 
	
	for (var key in config.shards) {
		config.shards[key] = _.extend({}, defaultConfig, config.shards[key]);
	}
};
...
ConnectionPool.prototype.getPool = function(name) {
  ...
  var pool = Pooling.Pool({
    ...
    create: function(callback) {
      var config = self.config.shards[name];
